...
input_nodes_env = {"BipedalWalker-v3": 24, "LunarLanderContinuous-v2": 8, "Acrobot-v1": 6}
...
def _eval_genomes(eval_single_genome, genomes, neat_config):
    ...
    # Save best genome stats to log file
    genome = max(genomes, key=lambda x: x[1].fitness)[1]
    input_nodes = input_nodes_env[ENVIRONMENT_NAME]
    graph = neat_graphs.NeatGraph(genome, input_nodes=input_nodes)

    with open(LOGS_PATH,"a") as f:
        if q_importance == None:
            f.write("ID: " + str(genome.key) + ", fitness: " + str(genome.fitness) + " Q-Score: " + str(graph.q_score) + ", groups: " +  str(graph.groups) + " \n")
        else:
            #net = neat.nn.FeedForwardNetwork.create(genome, neat_config)
            actual_fitness = (genome.fitness - (100*q_importance)*graph.q_score) / (1 + q_importance*graph.q_score) # assumes in [-100, 300] range, reasonable for best performing network
            f.write("ID: " + str(genome.key) + ", fitness: " + str(genome.fitness) + ", actual fitness: " + str(actual_fitness) + ", Q-Score: " + str(graph.q_score) + ", groups: " +  str(graph.groups) + " \n")
...
def _parse_args():
    ...
    parser.add_argument('--checkpoint', nargs='?', default=r"Checkpoints\cp_config-bipedal.txt_2021_10_09_15_43_20_gen_280",
...
def run(eval_network, eval_single_genome, environment_name, environment, config_file):
    ...
    if CHECKPOINT_PREFIX is None:
        ...
        CHECKPOINT_PREFIX = "Checkpoints/cp_" + CONFIG_FILENAME.lower() + "_" + timestamp + "_gen_"

    if LOGS_PATH is None:
        timestamp = datetime.datetime.utcnow().strftime('%Y_%m_%d_%H_%M_%S')
        LOGS_PATH = "Logs/log_" + CONFIG_FILENAME.lower() + "_" + timestamp + ".txt"
